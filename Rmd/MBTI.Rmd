...
MBTI je test ličnosti koji kategorizira pojedince prema četiri dimenzije (Ekstroverzija/Introverzija, Senzornost/Intuicija, Razmišljanje/Osjećanje, Prosudba/Spontanost), dodjeljujući im jedan od 16 osobnostnih tipova.
...
\
\
Vizualiziramo podatke iz stupca "POSTURE" u odnosu na podatke iz stupca "GROUP".
```{r}
...
chisq
```
Provedbom hi kvadrat testa dobivamo iznimno maleni p-value, iz čega slijedi da odbacujemo nultu hipotezu uz razinu značajnosti od 0.05 te zaključujemo da su ekstrovertnost i način držanja zavisne varijable
...
chisq
```
Hi kvadrat test nam u ovom slučaju daje p-value od 0.7, iz čega slijedi da ne možemo odbaciti nultu hipotezu uz razinu značajnosti od 0.05 te zaključujemo da raspoznavanje/intuicija i način držanja ne ovise jedno o drugome.
...
Provest ćemo hi kvadrat test za kategorijske podatke gdje ćemo proučavati gore navedene stupce.
```{r}
...
Postavljamo hipoteze, neka je mi-i prosječna vrijednost visina i-te skupine. Osobnosti su grupirane u četiri skupine: "Analysts", "Diplomats", "Explorers" i "Sentinels".
\
\
H0: mi1 = mi2 = mi3 = mi4
...
Obzirom da u ovom koraku varijablu WEIGHT grupiramo u dvije grupe, težine ekstroverata i introverata.
Moramo provjeriti je li varijabla WEIGHT normalno distribuirana za svaku grupu.
```{r}
...
```{r}
print("Četvrti model za J i P karakteristike")
model <- lm(formula = dataset$J ~ dataset$HEIGHT + 
              dataset$SEX + 
              dataset$PAIN_3 + 
              dataset$PAIN_4+
...
model_summary <- summary(model)
r_squared <- model_summary$r.squared
print(paste("R kvadrat za predikciju slova J četvrtim modelom", r_squared))
...
model_summary <- summary(model)
r_squared <- model_summary$r.squared
print(paste("R kvadrat za predikciju slova P četvrtim modelom", r_squared))
```
Vidimo kako smo dobili veći r2 izbacivši neke neznačajne regresore, a ubacivši koeficijente za neke druge karakteristike. To nam govori da unatoč tome što bi karakteristike trebale biti neovisne jedna od druge, na temelju nekih je moguće bolje predvidjeti drugu.
\
Testiramo model
```{r}
model <- lm(formula = dataset$J ~ dataset$HEIGHT + 
...
Kako bismo testirali je li postotak ekstroverata isti kod mladjih i starijih ljudi, koristit ćemo Z-test o dvije proporcije, uz alfa=0.05, a hipoteze postavljamo ovako
\
\
